...
#     filed_trip_set.add(student)
# print(f'조사된 수학여행지는 {filed_trip_set}입니다.')
# print(f'조사된 수학여행지는 {list(filed_trip_set)}입니다.')
'''
짝수만 추출하기
...
# list_nums = []
# list_even = []
# choice_num = int(input('몇 개의 숫자를 입력할까요? >>> '))
# for i in range(choice_num):
#     num = int(input(f'{i+1}번째 숫자를 입력하세요 >>>'))
#     list_nums.append(int(num))
#     if num % 2 == 0:
#         list_even.append(int(num))
# print(f'입력 받은 숫자는 {list_nums}입니다')
# print(f'입력 받은 숫자는 {list_even}입니다')
...
def add_numbers():
    numbers1 = []
    select_num = int(input('숫자 몇 까지 입력하시겠습니까? >>> '))
    for i in range(select_num):
        numbers1.append(i+1)


def add_number1(last_num):
    numbers1 = []
...
def add_numbers3(last_num, str1):
    numbers1 = []
    # result = list(range(1, last_num + 1)) + str1
...
def count_even_odd2(numbers):
    ...
    for number in numbers:
        if number % 2 == 0:
            even += 1
    print(f'짝수의 개수 : {even}')
    print(f'홀수의 개수 : {len(numbers) - len(even)}')