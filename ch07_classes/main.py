...
# 클래스 정의 예시
class WaffleMachine :
    pass                    # 이거는 클래스나 함수에 쓰면 엔터쳤을 때 오류 안남

# 객체 생성 예시
waffle = WaffleMachine()
print(waffle)   # 결과 값 : <__main__.WaffleMachine object at 0x102fd1580>
'''
클래스의 구성

1. 클래스의 기본 구성
    객체를 만들어내는 클래스는 객체가 가져야 할 구성 요소를 지닙니다(Java 때 제가 방이 가져야 할 구성요소는 뭐였냐고 질문했었습니다)
    ...
    self 키워드 : 인스턴스 변수에서 각각의 객체를 의미하기 위해서 self 키워드를 붙여줍니다.
     인스턴스 메서드에서의 첫 번째 매개변수로 '항상' self를 추가해야합니다(그래서 자동 생성됩니다).
'''
# 클래스 정의
class Person:
...
person1.set_info(age=20, tel='010-1234-5678', address='부산과역시 부산진구', name='김일')
# 이상의 코드에서 keyword argument를 썼습니다. 어떤 속송에 어떤 값을 넣는지 아니까 굳이 Java의 builder 패턴이 필요 없겠네요.
person1.show_info()
print(person1.show_info2())

# constructor 디텍토리 -> main